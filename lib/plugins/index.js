...
let busy = false;
let PLUGIN_ROOT, PLUGIN_INDEX, PLUGIN_MANIFEST, plugins;
...
const runPlugins = (commands) => {
    PLUGIN_ROOT = new find_folder("plugins")[0] || utils.wd();
    PLUGIN_INDEX = `${PLUGIN_ROOT}/index.js`;
    PLUGIN_MANIFEST = `${PLUGIN_ROOT}/plugins.json`;

    plugins = JSON.parse(fs.readFileSync(PLUGIN_MANIFEST));
